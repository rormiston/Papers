...
def run_network(
    ...
    loss       = 'mae',
    lr         = None,
    model_dir  = '/home/rich.ormiston/DeepCleanTesting/SaveModel',
    momentum   = 0.0,
    nesterov   = False,
    optimizer  = 'adam',
    ...
    scaled     = std_scaler.fit_transform(values)
    ...
    print('  [+] Loading model')
    if load_model:
        if os.path.isfile('{0}/{1}_model.h5'.format(model_dir, loop)):
            model = keras.models.load_model('{0}_model.h5'.format(loop))
            ppr.alert('    - Loaded successfully\n', color='OKGREEN')
            not_loaded = False
        else:
            ppr.alert('    - ERROR: Attempted to load a model that does not exist',
                      color='FAIL')
    ...
    if not_loaded:
        ...
        ppr.alert('    - Model created', color='OKGREEN')
    ...
    if save_model:
        model.save('{0}/{1}_model.h5'.format(model_dir, loop))
...
if __name__ == '__main__':
    ...
    for ix in range(len(to_run)):
        ...
        if to_run[loop]:
            ...
            alert('\n{}'.format(text), color='UNDERLINE')
            predictions[loop], darm, fs = run_network(**param_dict[loop])
            runs.append(loop)

    if ix == range(len(to_run)) - 1:
        import gwpy
        gwpy_timeseries = gwpy.timeseries.TimeSeries(darm)
        gwpy_timeseries.write('cleaned_data.gwf')
